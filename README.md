...
## Conceptual Model:
<img width="782" height="795" alt="image" src="https://github.com/user-attachments/assets/cdf63bd0-6bcd-4d8d-b150-f30ec0479707" />

The central entity in our model is order, which represents an individual order made by a customer, the order entity describes the attributes of the order itself, not of the products ordered, it is identified by an order ID. Each order has one employee who facilitates the order, and one employee can facilitate many orders, the employee table is identified by an employee ID. Each employee has one manager, and each manager can have many employees, the manager table is identified by the manager ID. Also, each order has one customer, and one customer can have many orders, the customer entity is identified by customer ID. Each order consists of many order lines, which are the descriptions of the pricing, quantity, and physical attributes of the products at the time it was ordered, each order line is identified by the order line ID. Each order line consists of one product, and one product can be a part of many order lines, the product table describes the attributes of the product as it currently stands, the product entity is identified by its sku. Each product is a part of one category, and a category can have many products, the category entity is identified by category ID. Also, each product is bought from one vendor, and one vendor can sell many products, the vendor entity is identified by the vendor ID. Finally, each product can be a variant of a parent product, and each parent product can have many variants, so the product entity has a recursive relationship creating the parent sku identifier for products that are variants of a parent product.
